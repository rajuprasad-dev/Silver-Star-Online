...
function profile_upload(input) {
	if (input.files && input.files.length === 1) {
		document.querySelector(".more_image_span")?.remove();

		var reader = new FileReader();
		reader.onload = function (e) {
			$("#profile_pic_preview").css(
				"background-image",
				"url(" + e.target.result + ")"
			);
		...
		reader.onload = function (e) {
			...
			$(".more_image_span").html(
				"+" + parseInt(input.files.length - parseInt(1)) + "<br>more"
			);
...
function product_upload(input) {
	if (input.files && input.files.length === 1) {
		...
		reader.onload = function (e) {
			...
			$(".product_pic_preview").append(more_image);
			$(".more_image_span").html(
				"+" + parseInt(input.files.length - parseInt(1)) + "<br>more"
			);
			$(".more_image_span").css("display", "flex");
...
if (document.getElementById("page_editor")) {
	ClassicEditor.create(document.querySelector("#page_editor"), {
		toolbar: {
			items: [
				"heading",
				"|",
...
$(document).ready(function () {
	$("#details_table")
		...
		.buttons()
		.container()
		.appendTo("#details_table_wrapper .row:eq(0)");
});

// Auto Height Textareas
window.setTimeout(function () {
	$("textarea")
		.not("textarea#test_series_chapters_covered")
...
var test_series_course_select = document.getElementById(
	"test_series_course_select"
);
