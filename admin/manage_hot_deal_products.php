<?php
$page="Manage Hot Deals Products";
include("header.php");
...
$check = "SELECT product_id FROM featured_hotdeals WHERE `type` = '2'";
if($db->sql($check))
{
    $nr = $db->numrows();
    $res = $db->result();
...
<div class="container-fluid my-5">
    <div class="d-flex mt-4 justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <h2 class="h4">Manage Hot Deals Products</h2>
    </div>

    <div class="row card card-body border-light shadow-sm px-md-3 px-2">
        <div class="main_form_div p-0">
            <form id="add_featured_products_module" enctype="multipart/form-data" data-id="2">
                ...
                <div class="form-group mb-3 btn_group_div_main">
                    <input type="submit" name="add_featured" class="btn btn-primary submit_btn" value="Add Hot Deals Products">
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="d-flex mt-4 justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <div class="d-block mb-4 mb-md-0">
            <h2 class="h4">Added Hot Deals Products</h2>
            <p class="mb-0">All Your Added Hot Deals Products Will Appear Here.</p>
        </div>
    </div>
    <div class="card card-body border-light shadow-sm table-wrapper table-responsive py-4">
        ...
        $sql = "SELECT `featured_hotdeals`.*, `products`.`name` as product_name FROM featured_hotdeals LEFT JOIN products ON `featured_hotdeals`.`product_id` = `products`.`id` WHERE `featured_hotdeals`.`type` = 2 ORDER BY `products`.`id` DESC";
